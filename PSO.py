...
class PSO:
      def __init__(self,num=30):
          ...
          self.l1 = Label(right_f,font=('times new roman',12),text="Define Velocity Impact",bg='#ff4d4d')
          self.l1.pack()

          #Define Current Velocity Impact
          self.VI = DoubleVar()
          scale = Scale(right_f, variable = self.VI, from_=0, to=1,resolution = 0.1,  orient=HORIZONTAL)
          scale.pack(anchor=CENTER)

          self.l2 = Label(right_f,font=('times new roman',12),text="Define Personal Impact",bg='#ff4d4d')
          self.l2.pack()

          #Define Current Velocity Impact
          self.PI = DoubleVar()
          scale = Scale(right_f, variable = self.PI, from_=0, to=1,resolution = 0.1, orient=HORIZONTAL)
          scale.pack(anchor=CENTER)
        
          self.l3 = Label(right_f,font=('times new roman',12),text="Define Global Impact",bg='#ff4d4d')
          self.l3.pack()
          
          #Define Current Velocity Impact
          self.GI = DoubleVar()
          scale = Scale(right_f, variable = self.GI, from_=0, to=1,resolution = 0.1, orient=HORIZONTAL)
          scale.pack(anchor=CENTER)
      ...
      def illustrate_pso(self,event):
            ...
            #self.PI.get()       # Exploration   0.1
 
            #Define Global Impact
            #self.GI.get()       # Exploitation  0.8
            ...
            for i in range (0,50):
                  for i in range(self.n):
                       ...
                       #Calcular Velocidad
                        for i in range(self.n):
                              #Velocidad en X
                              self.v[0][i] = self.VI.get() * self.v[0][i] + (self.Pbest[0][i] - self.a[0][i]) * rand.random() * self.PI.get() + (self.Gbest[0][0] - self.a[0][i]) * rand.random() * self.GI.get()
                              self.a[0][i] = self.a[0][i] + self.v[0][i]
                              #Velocidad en Y
                              self.v[1][i] = self.VI.get() * self.v[1][i] + (self.Pbest[1][i] - self.a[1][i]) * rand.random() * self.PI.get() + (self.Gbest[0][1] - self.a[1][i]) * rand.random() * self.GI.get()
